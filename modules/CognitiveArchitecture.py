...
class CognitiveArchitecture(brica1.Module):
    def __init__(self, rl, train, modelp=False, config=None):
        ...
        self.fef = FEF(observation_dim,action_dim, token_dim,rl, train)
        self.sc = SC(action_dim=action_dim, token_dim=token_dim)
        self.lip = LIP(observation_dim=observation_dim, token_dim=token_dim)
        self.retina = Retina(observation_dim=observation_dim, token_dim=token_dim, modelp=modelp, config=config["retina"])
